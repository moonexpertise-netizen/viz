...
import { reportAPI, multiperiodAPI, templateAPI } from '../services/api';
...
function CashFlowDetailModal({ balanceId, clientId, category, categoryLabel, month, onClose }) {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      try {
        ...
        let res;
        if (clientId) {
          res = await reportAPI.getClientCashFlowEntries(clientId, category, month || undefined, month || undefined);
        } else {
          res = await reportAPI.getCashFlowEntries(balanceId, category, month || undefined, month || undefined);
        }
    ...
    fetchEntries();
  }, [balanceId, clientId, category, month]);
...
export default function MonthlyView() {
  const { balanceId, clientId } = useParams();
  ...
  const storageKey = `moon_prefs_${clientId || balanceId || 'default'}`;
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(savedPrefs.selectedTemplate || 'default');
  const [customPLData, setCustomPLData] = useState(null);
  ...
  useEffect(() => {
    const prefs = { activeTab, decimals, granularity, selectedTemplate, fromMonth, toMonth };
    try { localStorage.setItem(storageKey, JSON.stringify(prefs)); } catch {}
  }, [activeTab, decimals, granularity, selectedTemplate, fromMonth, toMonth, storageKey]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let res;
        if (isClientMode) {
          res = await multiperiodAPI.getClientMonthly(clientId);
          setData(res.data);
        } else {
          res = await reportAPI.getReports(balanceId);
          setData(res.data);
        }
      } catch (err) {
    ...
    fetchData();
  }, [balanceId, clientId, isClientMode]);

  // Load templates on mount
  useEffect(() => {
    templateAPI.getTemplates().then(res => setTemplates(res.data.templates || [])).catch(() => {});
  }, []);

  // When template changes, apply it
  useEffect(() => {
    if (!clientId || selectedTemplate === 'default') {
      setCustomPLData(null);
      return;
    }
    templateAPI.applyMonthly(selectedTemplate, clientId)
      .then(res => setCustomPLData(res.data))
      .catch(() => setCustomPLData(null));
      ...
      <header className="bg-navy text-white">
        <div className="max-w-full mx-auto px-6 py-4 flex items-center justify-between">
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-3 hover:opacity-80 transition">
            <img src="/moon-logo.png" alt="MOON" className="h-9 w-9 rounded-lg" />
            <div className="text-left">
              <h1 className="font-display text-xl font-semibold tracking-wide text-white">
                {clientName || 'MOON Insight'}
              </h1>
              {periodLabel && (
                <p className="text-sage/70 text-xs font-display">{periodLabel}</p>
              )}
            </div>
          </button>
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/dashboard')} className="text-sage hover:text-white text-sm transition font-display">
              Mes dossiers
            </button>
          </div>
        </div>
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-full mx-auto px-6">
          <div className="flex items-center justify-between py-2">
            ...
            {/* Template selector + Decimal toggle */}
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <select value={selectedTemplate} onChange={e => setSelectedTemplate(e.target.value)}
                  className="bg-slate-50 text-gray-700 text-xs rounded px-2 py-1 border border-slate-200">
                  <option value="default">PCG Standard</option>
                  {templates.filter(t => !t.builtin).map(t => (
                    <option key={t.id} value={t.id}>{t.name}</option>
                  ))}
                </select>
                {selectedTemplate !== 'default' && (
                  <button onClick={() => navigate(`/templates/edit/${selectedTemplate}?clientId=${clientId || ''}`)} className="text-xs text-indigo-600 hover:underline">
                    Modifier
                  </button>
                )}
                <button onClick={() => navigate(`/templates/new?clientId=${clientId || ''}`)} className="text-xs text-gray-500 hover:underline">
                  + Nouveau
                </button>
              </div>
              {/* Granularity toggle */}
